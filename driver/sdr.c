...
void set_new_ANS_mask(struct openwifi_priv *priv) {
	...
	*prev = *ctime;		
	...
	// +26 -> mask[58]
	clean_mask (m);	
	for (carrier = -28; carrier <= 28; carrier ++) {
		// we must skip DC
...
void ans_obfuscation(struct openwifi_priv *priv) {
...
//		printk("%16llx\n", (*(u64*)(&(dma_buf[i*8]))));
	...
	// ANS OBFUSCATION CONTROL ON TX
	ans_obfuscation(priv);
	...
	printk("rate_hw_value = %08X, phy_hdr_config = %08X\n", rate_hw_value, phy_hdr_config);
